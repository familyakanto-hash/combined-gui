# =============================================================================
#  RHA-Blended Concrete — RCPT (28-day) Durability Toolkit
#  Tab 1: Predict RCPT   (forward)  — mix in  -> Coulombs out
#  Tab 2: Design a mix   (inverse)  — target Coulombs in -> best mixes out
#
#  Best model from the thesis: Gradient Boosting (Optuna-tuned) on the
#  leakage-free pipeline:  median impute -> IQR winsorise (x1.5)
#                          -> Yeo-Johnson -> RobustScaler.
#  Run:  streamlit run rcpt_app.py
# =============================================================================
...
GB_PARAMS = dict(n_estimators=218, max_depth=3, learning_rate=0.08019523499097962,
                 subsample=0.5346997819228771, min_samples_leaf=3, random_state=SEED)
N_CANDIDATES = 40000

# ASTM C1202 classification thresholds (Coulombs)
ASTM_BANDS = [
    (4000, float("inf"), "High permeability"),
    (2000, 4000, "Moderate permeability"),
    (1000, 2000, "Low permeability"),
    (100, 1000, "Very low permeability"),
    (0, 100, "Negligible permeability"),
]


def classify_rcpt(value):
    for lo, hi, label in ASTM_BANDS:
        if lo <= value < hi:
            return label
    return "Negligible permeability"
...
@st.cache_resource(show_spinner="Training the model and preparing the toolkit "
                                 "(first run only)\u2026")
def setup():
    """Train Gradient Boosting once; build candidate pool for the inverse tab."""
    ...
    pipe = build_pipeline(GradientBoostingRegressor(**GB_PARAMS))
    pipe.fit(X_tr, y_tr)
    ...
    pool["predicted_rcpt"] = np.clip(pipe.predict(pool[FEATURES]), 0, None)

    return pipe, metrics, bounds, pool, sd


# ----------------------------------------------------------------------------- UI
st.set_page_config(page_title="RCPT | RHA Concrete Toolkit",
                   page_icon="\U0001F6E1\uFE0F", layout="centered")
inject_base_css()
back_to_home_button()

st.title("\U0001F6E1\uFE0F RCPT (28-day Durability)")
st.caption("Best model from the analysis: **Gradient Boosting (Optuna-tuned)** "
           "(leakage-free pipeline). Predict the 28-day Rapid Chloride "
           "Penetrability of a mix, or design a mix for a target Coulomb value. "
           "**Lower RCPT = better chloride resistance / durability.**")
...
with st.expander("Model performance (held-out test set)", expanded=False):
    ...
    c2.metric("RMSE", f"{metrics['RMSE']:.0f} C")
    c3.metric("MAE", f"{metrics['MAE']:.0f} C")
    c4.metric("Samples", f"{metrics['n']}")
    st.caption("80/20 split, seed 42 \u2014 consistent with the thesis methodology. "
               "RCPT is a harder property to predict than strength (durability is "
               "more sensitive to microstructure), hence the lower R\u00b2.")

with st.expander("ASTM C1202 classification reference", expanded=False):
    st.markdown(
...
tab_predict, tab_design = st.tabs(["\U0001F50E  Predict RCPT", "\U0001F9EE  Design a mix"])
...
with tab_predict:
    ...
    if st.button("Calculate RCPT", type="primary",
        ...
        band = classify_rcpt(pred)

        st.markdown(
            f"<div style='text-align:center'>"
            f"<div style='font-size:0.95rem;color:#666'>Predicted RCPT (28-day)</div>"
            f"<div style='font-size:3.2rem;font-weight:700;color:#1a5f9e;line-height:1.1'>"
            f"{pred:.0f} <span style='font-size:1.4rem'>Coulombs</span></div>"
            f"<div style='font-size:0.95rem;color:#333;margin-top:0.2rem'><b>{band}</b></div>"
            f"<div style='font-size:0.8rem;color:#888'>approx. \u00b1{metrics['RMSE']:.0f} "
            f"C (model RMSE)</div></div>", unsafe_allow_html=True)
...
# ============================================================ TAB 2: inverse
with tab_design:
    st.subheader("What RCPT do you need?")
    st.caption("Remember: for durability, **lower is better**.")
    s_min = float(pool["predicted_rcpt"].quantile(0.01))
    s_max = float(pool["predicted_rcpt"].quantile(0.99))

    target = st.number_input(
        "Target RCPT (Coulombs)",
        min_value=round(s_min, 0), max_value=round(s_max, 0),
        value=float(np.clip(1000.0, s_min, s_max)), step=50.0,
        help=f"Achievable range for this dataset is roughly {s_min:.0f}\u2013{s_max:.0f} C.",
        key="inv_target",
    )
    ...
    objective = c1.selectbox(
        "What does \u201cbest\u201d mean?",
        ["Maximum durability (lowest RCPT)",
    ...
    tol = c2.slider("Tolerance (\u00b1 Coulombs)", 25.0, 500.0, 150.0, 25.0, key="inv_tol")
    ...
    if st.button("Find best mixes", type="primary", use_container_width=True,
                 key="btn_design"):
        hits = pool[(pool["predicted_rcpt"] - target).abs() <= tol].copy()
        note = None
        if hits.empty:
            hits = pool.iloc[(pool["predicted_rcpt"] - target).abs()
                             .argsort()[:300]].copy()
            note = (f"No mix landed within \u00b1{tol:.0f} C of {target:.0f} C \u2014 "
                    "showing the closest achievable mixes instead.")

        if objective == "Maximum durability (lowest RCPT)":
            hits = hits.sort_values("predicted_rcpt", ascending=True)
        elif objective == "Most sustainable (max RHA, least cement)":
            ...
            hits = hits.reindex((hits["predicted_rcpt"] - target).abs()
                                .sort_values().index)
        ...
        for _, r in hits.iterrows():
            ...
            if all((np.abs(rv - p[:-1]) / sd.values).sum() > 1.0 for p in picked):
                picked.append(np.append(rv, r["predicted_rcpt"]))
                if len(picked) >= k:
                    break

        result = pd.DataFrame(picked, columns=FEATURES + ["predicted_rcpt"])
        result.insert(0, "Option", [f"Mix {i+1}" for i in range(len(result))])
        result["Class"] = result["predicted_rcpt"].apply(classify_rcpt)
        ...
        show.columns = (["Option"] + [LABELS[f] for f in FEATURES]
                        + ["Predicted RCPT (C)", "Class"])
        num_cols = [c for c in show.columns if c not in ("Option", "Class")]
        st.dataframe(
        ...
        st.success(
            ...
            f"\u2192 predicted **{best['predicted_rcpt']:.0f} Coulombs** "
            f"({classify_rcpt(best['predicted_rcpt'])})."
        )
        st.download_button(
            "Download these mixes (CSV)",
            result.to_csv(index=False).encode(),
            file_name=f"rcpt_mixes_{int(target)}C.csv", mime="text/csv",
            key="dl_design",
        )
...
st.caption("Predictions are for preliminary screening within the training domain and do "
           ...
           "RCPT value, so the designer returns a diverse ranked set, not a single answer. "
