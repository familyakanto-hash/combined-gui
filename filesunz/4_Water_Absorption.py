# =============================================================================
#  RHA-Blended Concrete — Water Absorption (28-day) Toolkit
#  Tab 1: Predict absorption (forward)  — mix in  -> % absorption out
#  Tab 2: Design a mix       (inverse)  — target absorption in -> best mixes out
#
#  Best model from the thesis: Gradient Boosting (Optuna-tuned) on a
#  leakage-free pipeline:  median impute -> IQR winsorise (x1.5)
#                          -> Yeo-Johnson -> RobustScaler.
#  Run:  streamlit run water_absorption_app.py
# =============================================================================
...
GB_PARAMS = dict(n_estimators=295, max_depth=4, learning_rate=0.14789038389443718,
                 subsample=0.8510627567665825, min_samples_leaf=13, random_state=SEED)
N_CANDIDATES = 40000
...
def setup():
    ...
    pool["predicted_absorption"] = np.clip(pipe.predict(pool[FEATURES]), 0, None)

    return pipe, metrics, bounds, pool, sd


# ----------------------------------------------------------------------------- UI
st.set_page_config(page_title="Water Absorption | RHA Concrete Toolkit",
                   page_icon="\U0001F4A7", layout="centered")
inject_base_css()
back_to_home_button()

st.title("\U0001F4A7 Water Absorption (28-day)")
st.caption("Best model from the analysis: **Gradient Boosting (Optuna-tuned)** "
           "(leakage-free pipeline). Predict the 28-day water absorption of a mix, "
           "or design a mix for a target absorption. **Lower absorption = better "
           "durability.**")
...
tab_predict, tab_design = st.tabs(["\U0001F50E  Predict absorption", "\U0001F9EE  Design a mix"])
...
with tab_predict:
    ...
    if st.button("Calculate water absorption", type="primary",
        ...
        st.markdown(
            ...
            f"<div style='font-size:0.95rem;color:#666'>Predicted 28-day water absorption</div>"
            f"<div style='font-size:3.2rem;font-weight:700;color:#1a5f9e;line-height:1.1'>"
            f"{pred:.2f} <span style='font-size:1.4rem'>%</span></div>"
            f"<div style='font-size:0.8rem;color:#888'>approx. \u00b1{metrics['RMSE']:.2f} "
            f"% (model RMSE)</div></div>", unsafe_allow_html=True)
...
# ============================================================ TAB 2: inverse
with tab_design:
    st.subheader("What absorption do you need?")
    st.caption("Remember: for durability, **lower is better**.")
    s_min = float(pool["predicted_absorption"].quantile(0.01))
    s_max = float(pool["predicted_absorption"].quantile(0.99))

    target = st.number_input(
        "Target water absorption (%)",
        min_value=round(s_min, 1), max_value=round(s_max, 1),
        value=float(np.clip(4.0, s_min, s_max)), step=0.1,
        help=f"Achievable range for this dataset is roughly {s_min:.1f}\u2013{s_max:.1f} %.",
        key="inv_target",
    )
    ...
    objective = c1.selectbox(
        "What does \u201cbest\u201d mean?",
        ["Maximum durability (lowest absorption)",
    ...
    tol = c2.slider("Tolerance (\u00b1 %)", 0.1, 1.5, 0.4, 0.05, key="inv_tol")
    ...
    if st.button("Find best mixes", type="primary", use_container_width=True,
                 key="btn_design"):
        hits = pool[(pool["predicted_absorption"] - target).abs() <= tol].copy()
        note = None
        if hits.empty:
            hits = pool.iloc[(pool["predicted_absorption"] - target).abs()
                             .argsort()[:300]].copy()
            note = (f"No mix landed within \u00b1{tol:.2f}% of {target:.1f}% \u2014 "
                    "showing the closest achievable mixes instead.")

        if objective == "Maximum durability (lowest absorption)":
            hits = hits.sort_values("predicted_absorption", ascending=True)
            ...
            hits = hits.reindex((hits["predicted_absorption"] - target).abs()
                                .sort_values().index)
        ...
        for _, r in hits.iterrows():
            ...
            if all((np.abs(rv - p[:-1]) / sd.values).sum() > 1.0 for p in picked):
                picked.append(np.append(rv, r["predicted_absorption"]))
                if len(picked) >= k:
                    break

        result = pd.DataFrame(picked, columns=FEATURES + ["predicted_absorption"])
        ...
        st.success(
            ...
            f"\u2192 predicted **{best['predicted_absorption']:.2f}%** water absorption."
        )
        st.download_button(
            "Download these mixes (CSV)",
            result.to_csv(index=False).encode(),
            file_name=f"water_absorption_mixes_{target:.1f}pct.csv", mime="text/csv",
            key="dl_design",
        )
...
st.caption("Predictions are for preliminary screening within the training domain and do "
           ...
           "absorption, so the designer returns a diverse ranked set, not a single answer. "
